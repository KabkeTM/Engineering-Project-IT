...
{
	[Serializable]
	public class PaletteGrading_Red
	{
		public int size = 6;
	...
	public class RetroPalette_Red : MonoBehaviour
	{
		public List<PaletteGrading_Red> gradings = new List<PaletteGrading_Red>();
		public Color[] colors;
		...
		public void BlendStops()
		{
			...
			{
				PaletteGrading_Red pgBefore = i == 0 ? null : gradings[i - 1];
				PaletteGrading_Red pgNow = gradings[i];
				PaletteGrading_Red pgAfter = i == gradings.Count - 1 ? null : gradings[i + 1];
		...
		public void OnRenderImage(RenderTexture src, RenderTexture dest)
		{
			...
			int s = 0;
			foreach (PaletteGrading_Red pg in gradings)
				s += pg.size;
